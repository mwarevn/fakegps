...
class MapActivity : BaseMapActivity(), OnMapReadyCallback, GoogleMap.OnMapClickListener, GoogleMap.OnMarkerDragListener {
    ...
    override fun moveMapToNewLocation(moveNewLocation: Boolean, shouldMark: Boolean) {
        if (moveNewLocation && ::mMap.isInitialized) {
            val newPos = LatLng(lat, lon); mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(newPos, 16f))
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        mMap = googleMap
        if (checkPermissions()) { mMap.isMyLocationEnabled = true; getLastLocation() } 
        ...
        mMap.uiSettings.apply { isMyLocationButtonEnabled = false; isZoomControlsEnabled = false; isCompassEnabled = true }
        mMap.setOnMapClickListener(this); mMap.setOnMarkerDragListener(this)
        mMap.setOnCameraMoveListener { PrefManager.cameraBearing = mMap.cameraPosition.bearing }
        setupButtons(); setupSearchBoxes(); restoreFakeLocationMarker()
        if (isBound) syncWithService()
    }

    private fun syncWithService() {
        val service = locationService ?: return
        if (service.isDrivingFlow.value && ::mMap.isInitialized) {
            ...
            if (routePoints.isNotEmpty()) {
                startMarker?.remove()
                startMarker = mMap.addMarker(MarkerOptions().position(routePoints.first()).title("Bắt đầu").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_GREEN)))
                destMarker?.remove()
                destMarker = mMap.addMarker(MarkerOptions().position(service.currentTargetLatLng ?: routePoints.last()).title("Đích"))
            }
    ...
    override fun setupButtons() {
        ...
        binding.actionButton.setOnClickListener {
            when (currentMode) {
                ...
                AppMode.ROUTE_PLAN -> if (startMarker != null && destMarker != null) checkAndStartNavigation() else showToast("Vui lòng chọn đủ điểm đi và đến")
        ...
        binding.setLocationButton.setOnClickListener {
            if (isGpsSet) {
                fakeLocationCircle?.remove(); fakeLocationCircle = null; fakeLocationCenterDot?.remove(); fakeLocationCenterDot = null
        ...
        binding.addFavoriteButton.setOnClickListener { destMarker?.position?.let { showAddFavoriteDialog(it) } }
    ...
    private suspend fun searchLocation(query: String, onFound: (LatLng) -> Unit) {
        withContext(Dispatchers.IO) {
            try {
                ...
                if (!addresses.isNullOrEmpty()) { val pos = LatLng(addresses[0].latitude, addresses[0].longitude); withContext(Dispatchers.Main) { onFound(pos) } } 
                else { withContext(Dispatchers.Main) { showToast("Không tìm thấy địa điểm") } }
            } catch (e: Exception) { withContext(Dispatchers.Main) { showToast("Lỗi tìm kiếm: ${e.message}") } }
        }
    }
    ...
    private fun setDestinationMarker(position: LatLng) {
        destMarker?.remove(); destMarker = mMap.addMarker(MarkerOptions().position(position).title("Điểm đến").draggable(true))
        ...
        routePolyline?.remove(); routePolyline = null; updateSwapButtonVisibility(); updateReplaceLocationButtonVisibility()
    }

    private fun updateActionButtonsVisibility() {
        if (currentMode != AppMode.SEARCH) return
        val markerPos = destMarker?.position; val fakePos = currentFakeLocationPos
        ...
        if (isAtFakeLocation) { binding.actionButton.visibility = View.GONE; binding.cancelRouteButton.visibility = View.GONE; destMarker?.remove(); destMarker = null; binding.destinationSearch.text.clear() } 
        else if (destMarker == null) { binding.actionButton.visibility = View.GONE; binding.cancelRouteButton.visibility = View.GONE } 
        else { binding.actionButton.apply { text = "Chỉ đường"; visibility = View.VISIBLE; setIconResource(R.drawable.ic_navigation) }; binding.cancelRouteButton.apply { text = "Huỷ"; visibility = View.VISIBLE }; destMarker?.isVisible = true }
    ...
    private fun clearDestinationMarker() {
        destMarker?.remove(); destMarker = null; binding.destinationSearch.text.clear()
    ...
    private fun setStartMarker(position: LatLng) {
        startMarker?.remove(); startMarker = mMap.addMarker(MarkerOptions().position(position).title("Điểm bắt đầu").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_GREEN)).draggable(true))
        lifecycleScope.launch { binding.startSearch.setText(getAddressFromLocation(position)) }
        destMarker?.let { val bounds = LatLngBounds.builder().include(position).include(it.position).build(); mMap.animateCamera(CameraUpdateFactory.newLatLngBounds(bounds, 150)); drawRoute() }
    ...
    private fun updateSwapButtonVisibility() { binding.swapButtonContainer.visibility = if (startMarker != null && destMarker != null && currentMode != AppMode.NAVIGATION) View.VISIBLE else View.GONE }
    ...
    private fun updateUseCurrentLocationButtonVisibility() {
        ...
        val startPos = startMarker?.position; val isSame = startPos != null && Math.abs(startPos.latitude - currentFakeLocationPos!!.latitude) < 1e-4 && Math.abs(startPos.longitude - currentFakeLocationPos!!.longitude) < 1e-4
    ...
    private fun updateMarkersDraggableState() { destMarker?.isDraggable = currentMode != AppMode.NAVIGATION; startMarker?.isDraggable = currentMode == AppMode.ROUTE_PLAN }

    private fun swapStartAndDestination() {
        if (startMarker == null || destMarker == null) return
        val sPos = startMarker!!.position; val dPos = destMarker!!.position; val sText = binding.startSearch.text.toString(); val dText = binding.destinationSearch.text.toString()
        startMarker?.position = dPos; destMarker?.position = sPos; binding.startSearch.setText(dText); binding.destinationSearch.setText(sText)
        if (currentMode == AppMode.ROUTE_PLAN) drawRoute()
    }

    private fun drawRoute() {
        val start = startMarker?.position ?: return; val dest = destMarker?.position ?: return
        ...
        lifecycleScope.launch {
            try {
                ...
                routePolyline?.remove(); routePolyline = mMap.addPolyline(PolylineOptions().addAll(routePoints).color(Color.parseColor(ROUTE_COLOR)).width(ROUTE_WIDTH))
                binding.routeLoadingCard.visibility = View.GONE; binding.actionButton.apply { text = "Bắt đầu"; setIconResource(R.drawable.ic_navigation); visibility = View.VISIBLE }; binding.cancelRouteButton.apply { text = "Huỷ"; visibility = View.VISIBLE }
    ...
    private fun cancelRoutePlan() {
        routePolyline?.remove(); routePolyline = null; startMarker?.remove(); startMarker = null
    ...
    private fun handleNavigationUpdate(pos: LatLng) {
        ...
        fakeLocationCircle?.center = pos; fakeLocationCenterDot?.position = pos; updateCompletedPath(pos)
        if (isCameraFollowing && time - lastCameraUpdateTime >= CAMERA_UPDATE_INTERVAL_MS) { mMap.animateCamera(CameraUpdateFactory.newLatLng(pos)); lastCameraUpdateTime = time }
    ...
    private fun updateSetLocationButton() { binding.setLocationButton.setImageResource(if (isGpsSet) R.drawable.ic_gps_off else R.drawable.ic_location_on); binding.getFakeLocation.visibility = if (isGpsSet) View.VISIBLE else View.GONE; updateReplaceLocationButtonVisibility() }
    ...
    private fun updateReplaceLocationButtonVisibility() { binding.replaceLocationButton.visibility = if (currentMode == AppMode.SEARCH && isGpsSet && destMarker != null && currentFakeLocationPos != destMarker?.position) View.VISIBLE else View.GONE }

    private fun updateFakeLocationMarker(position: LatLng) { fakeLocationCircle?.remove(); fakeLocationCenterDot?.remove(); fakeLocationCircle = createStationaryLocationCircle(position) }
    ...
    private fun updateCompletedPath(pos: LatLng) {
        if (completedPathPoints.isEmpty() || distanceBetween(completedPathPoints.last(), pos) >= 5.0) {
            ...
            if (completedPathPoints.size % 3 == 0) {
                if (completedPolyline == null) { completedPolyline = mMap.addPolyline(PolylineOptions().addAll(completedPathPoints).color(Color.parseColor(COMPLETED_ROUTE_COLOR)).width(ROUTE_WIDTH + 1)) } 
                else { completedPolyline?.points = completedPathPoints }
            }
        }
    }
    ...
    private fun createFakeLocationCircle(center: LatLng, stroke: Int, fill: Int, centerColor: Int): Circle {
        fakeLocationCenterDot?.remove(); fakeLocationCenterDot = mMap.addMarker(MarkerOptions().position(center).icon(createCenterDotIcon(centerColor)).anchor(0.5f, 0.5f).zIndex(CENTER_DOT_Z_INDEX))
    ...
    private fun createCenterDotIcon(color: Int): BitmapDescriptor {
        val size = 48; val bitmap = Bitmap.createBitmap(size, size, Bitmap.Config.ARGB_8888); val canvas = Canvas(bitmap); val paint = Paint(Paint.ANTI_ALIAS_FLAG)
        paint.color = Color.WHITE; canvas.drawCircle(size / 2f, size / 2f, size / 2f, paint); paint.color = color; canvas.drawCircle(size / 2f, size / 2f, size * 0.375f, paint)
    ...
    private fun onNavigationComplete() {
        ...
        val finalPos = currentNavigationPosition ?: routePoints.lastOrNull() ?: destMarker?.position
        if (finalPos != null) { 
    ...
    private fun resetToSearchMode() {
        destMarker?.remove(); destMarker = null; startMarker?.remove(); startMarker = null; routePolyline?.remove(); routePolyline = null
        completedPolyline?.remove(); completedPolyline = null; completedPathPoints.clear(); fakeLocationCircle?.remove(); fakeLocationCircle = null
        fakeLocationCenterDot?.remove(); fakeLocationCenterDot = null; routePoints = emptyList(); currentMode = AppMode.SEARCH; hasSelectedStartPoint = false
    ...
    override fun onMarkerDragEnd(marker: Marker) {
        ...
        lifecycleScope.launch {
            ...
            if (marker == destMarker) { binding.destinationSearch.setText(addr); updateReplaceLocationButtonVisibility(); updateAddFavoriteButtonVisibility(); updateActionButtonsVisibility() } 
            else if (marker == startMarker) { binding.startSearch.setText(addr); updateUseCurrentLocationButtonVisibility() }
    ...
    private fun onFinishNavigation() {
        ...
        routePolyline?.remove(); routePolyline = null; completedPolyline?.remove(); completedPolyline = null; completedPathPoints.clear()
        startMarker?.remove(); startMarker = null; destMarker?.remove(); destMarker = null; routePoints = emptyList(); binding.destinationSearch.text.clear(); binding.startSearch.text.clear()
    ...
    private fun onRestartNavigation() { binding.completionActionsCard.visibility = View.GONE; isDriving = false; isPaused = false; completedPathPoints.clear(); fakeLocationCircle?.remove(); fakeLocationCircle = null; fakeLocationCenterDot?.remove(); fakeLocationCenterDot = null; if (routePoints.isNotEmpty()) startNavigation() else showToast("Dữ liệu đã mất") }
